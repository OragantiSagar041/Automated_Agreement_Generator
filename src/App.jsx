...
function App() {
  ...
  const filteredEmployees = employees.filter(emp => {
    ...
    const percentage = String(comp.percentage || "");
  ...
  const stats = {
    ...
    sent: employees.filter(e => e.status === 'Agreement Sent').length,
    pending: employees.length - employees.filter(e => e.status === 'Agreement Sent').length
  };
  ...
  return (
    <div className="container" style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '4rem' }}>
        {[
          { label: 'Total Companies', val: stats.total, color: 'var(--accent-color)', icon: '🏢' },
          { label: 'Agreements Sent', val: stats.sent, color: 'var(--success-text)', icon: '🚀' },
              ...
              viewMode === 'grid' ? (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                      ...
                      <div>
                        ...
                        <p style={{ margin: 0, color: 'var(--accent-color)', fontSize: '0.85rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{emp.email}</p>
                      </div>
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <span style={{ background: 'var(--bg-tertiary)', color: 'var(--text-secondary)', padding: '6px 12px', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 700 }}>{emp.compensation?.percentage}% Share</span>
                    {emp.status === 'Agreement Sent' && <span style={{ background: 'var(--success-bg)', color: 'var(--success-text)', padding: '6px 12px', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 800 }}>✅ DISPATCHED</span>}
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      width: '100%',
                      background: emp.status === 'Agreement Sent' ? 'transparent' : 'var(--accent-gradient)',
                      border: emp.status === 'Agreement Sent' ? `2px solid var(--accent-color)` : 'none',
                      color: emp.status === 'Agreement Sent' ? 'var(--accent-color)' : 'white',
                      padding: '16px', borderRadius: '15px', fontWeight: '900', fontSize: '1rem', cursor: 'pointer', transition: 'all 0.2s',
                      boxShadow: emp.status === 'Agreement Sent' ? 'none' : '0 4px 15px rgba(99, 102, 241, 0.4)'
                    }}
                  >
                    {emp.status === 'Agreement Sent' ? 'VIEW / RESEND' : 'GENERATE OFFER'}
                  </button>
                </motion.div>
              ) : (
                // --- LIST ITEM ---
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flex: 2 }}>
                    ...
                    <div>
                      ...
                      <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.85rem' }}>{emp.email}</p>
                    </div>
                  </div>

                  <div style={{ flex: 1, color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.9rem' }}>
                    {emp.compensation?.percentage}% Share
                  </div>

                  <div style={{ flex: 1 }}>
                    {emp.status === 'Agreement Sent' ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      minWidth: '140px',
                      boxShadow: emp.status === 'Agreement Sent' ? 'none' : '0 4px 10px rgba(99, 102, 241, 0.3)'
                    }}
                  >
                    {emp.status === 'Agreement Sent' ? 'VIEW / RESEND' : 'Generate'}
                  </button>
                </motion.div>
              )
            ))}
